...
(define complex->double-hummer
  (let ()
	...
	(list* (make-qa0-operation attr* 'dh-cmadd-b output*
	       ...
	       r*)))
    (define (dho-c2mul attr* output* input* r*)
      (check-output* 'complex-mul output* 1)
      (check-input* 'complex-mul input* 2)
      (let ([x (fresh-reg)])
	(list* (make-qa0-operation attr* 'dh-c2mul-b output* (cons x input*))
	       (make-qa0-operation attr* 'dh-c2mul-a (list x) input*)
	       r*)))
    (define (dho-c2madd attr* output* input* r*)
      (check-output* 'complex-mul output* 1)
      (check-input* 'complex-mul input* 3)
      (let ([x (fresh-reg)])
	(list* (make-qa0-operation attr* 'dh-c2madd-b output*
				   (cons x (cdr input*)))
	       (make-qa0-operation attr* 'dh-c2madd-a (list x) input*)
	       r*)))
	...
	(list* (make-qa0-operation attr* 'dh-dot-b output*
	       ...
	       r*)))
    (define (dho-dcmadd-fini attr* output* input* r*)
      (check-output* 'complex-real-cmul-conj-fini output* 1)
      (check-input* 'complex-real-cmul-conj-fini output* 1)
      (let ([r-x (fresh-reg)]
	    [r-i (fresh-reg)])
	(list* (make-qa0-operation attr* 'double-add output*
				   (list r-x r-i))
	       (make-qa0-operation attr* 'dh-imag (list r-i) input*)
	       (make-qa0-operation attr* 'dh-real (list r-x) input*)
	       r*)))
    ...
    (define dh-optable
      (list
       ...
       (list* 'complex-c2mul           #f                  dho-c2mul      )
       (list* 'complex-cmadd           #f                  dho-cmadd      )
       (list* 'complex-c2madd          #f                  dho-c2madd     )
       (list* 'complex-add-i           'dh-add-i           #f             )
       (list* 'complex-sub-i           'dh-sub-i           #f             )
       (list* 'complex-real-cmul-conj-init 'dh-dcmul       #f             )
       (list* 'complex-real-cmul-conj-add  'dh-dcmadd      #f             )
       (list* 'complex-real-cmul-conj-fini #f              dho-dcmadd-fini)
