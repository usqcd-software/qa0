  ...
  (let ()
    (define op-type-table
      '((dh-make                   dh-double)
        ...
        (dh-dot-add-a              dh-double)
        (dh-dot-add-b              dh-double)
        (dh-dot-add-i-a            dh-double)
        (dh-dot-add-i-b            dh-double)
        (dh-dot-sub-a              dh-double)
        (dh-dot-sub-b              dh-double)
        (dh-dot-sub-i-a            dh-double)
        (dh-dot-sub-i-b            dh-double)
        ...
        (dh-cmsub-a                dh-double)
        (dh-cmsub-b                dh-double)
        (dh-c2madd-a               dh-double)
        (dh-c2madd-b               dh-double)
        (dh-dcmul                  dh-double)
        (dh-dcmadd                 dh-double)
        (dh->float                 dh-double)
        (double-add                double)
    ...
    (define op-emit-table
      ...
      '((pointer-move            1 "$0 = %0"                                0)
        ...
        (dh-cmsub-a              3 "$0 = __fxcpnmsub(%0, %2, __creal(%1))"  4)
        (dh-cmsub-b              3 "$0 = __fxcxnpma(%0, %2, __cimag(%1))"   4)
        (dh-real-cmul-conj-init  0 "$0 = gZERO"                             0)
        (dh-real-cmul-conj-add   3 "$0 = __fpmadd(%0, %1, %2)"              4)
        (dh-real-cmul-conj-fini  1 "$0 = __creal(%0) + __cimag(%0)"         1)
        (dh-norm-init            0 "$0 = gZERO"                             0)
        (dh-norm-add             2 "$0 = __fpmadd(%0, %1, %1)"              4)
        (dh-norm-fini            1 "$0 = __creal(%0) + __cimag(%0)"         1)
        (dh-dot-init             0 "$0 = gZERO"                             0)
        (dh-dot-add-a            3 "$0 = __fxcpmadd(%0, %2, __creal(%1))"   4)
        (dh-dot-add-b            3 "$0 = __fxcxnsma(%0, %2, __cimag(%1))"   4)
        (dh-dot-add-i-a          3 "$0 = __fxcxnpma(%0, %2, __creal(%1))"   4)
        (dh-dot-add-i-b          3 "$0 = __fxcpmadd(%0, %2, __cimag(%1))"   4)
        (dh-dot-sub-a            3 "$0 = __fxcpnmsub(%0, %2, __creal(%1))"  4)
        (dh-dot-sub-b            3 "$0 = __fxcxnpma(%0, %2, __cimag(%1))"   4)
        (dh-dot-sub-i-a          3 "$0 = __fxcxnsma(%0, %2, __creal(%1))"   4)
        (dh-dot-sub-i-b          3 "$0 = __fxcpnmsub(%0, %2, __cimag(%1))"  4)
        (dh-dot-fini             1 "$0 = %0"                                0)
      ...
      (define (add-one name env)
        (if (not (memq name op-needing-one*)) env
            (ce-search-x env 'bgl/xlc 'one
                         (lambda (x) env)
                         (lambda ()
                           (ce-bind-x env 'bgl/xlc 'one (ck-new-var))))))
      (walk-code* code*
                  ;; operation
                  ...
                  (lambda (env var) env)
                  env))
    (define (bgl/xlc-back-end ast env)
      (values (complex->double-hummer ast) env))
