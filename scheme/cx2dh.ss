    ...
    (define (dho-madd attr* output* input* r*)
      ...
      (let ([x (fresh-reg)])
        (list* (make-qa0-operation attr* 'dh-madd-b output*
               ...
               r*)))
    (define (dho-msub attr* output* input* r*)
      (check-output* 'complex-mul output* 1)
      (check-input* 'complex-mul input* 3)
      (let ([x (fresh-reg)])
        (list* (make-qa0-operation attr* 'dh-msub-b output*
                                   (cons x (cdr input*)))
               (make-qa0-operation attr* 'dh-msub-a (list x) input*)
               r*)))
